...
async function get_all_users() {
  const result = await connectedKnex.raw("SELECT * FROM get_all_users()");
  return result.rows;
}

async function get_user_by_id(id) {
  const result = await connectedKnex.raw("SELECT * FROM get_user_by_id(?)", [
    id,
  ]);
  return result.rows;
}
...
async function new_user(new_user_data) {
  const result = await connectedKnex.raw("SELECT * FROM new_user(?, ?, ?, ?)", [
...
async function delete_user(id) {
  await connectedKnex.raw("CALL delete_user(?)", [id]);
}
